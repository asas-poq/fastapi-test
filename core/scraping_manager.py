...
class ScrapingManager:
    ...
    async def add_specific_anime(self, anime_slug):
        """
        Главный метод для скрапинга и добавления одного аниме в БД.
        """
        ...
        # 1. Получаем все эпизоды с jut.su
        seasons_with_links = await self.jutsu_scraper.get_all_episode_links_for_anime(anime_slug)
        if not seasons_with_links:
            return {"status": "error", "reason": "failed to get episode links"}

        # 2. Парсим первый эпизод, чтобы получить русское название для поиска
        ...
        async with async_playwright() as p:
            ...
            # Получаем базовую инфу с первого эпизода
            base_episode_data = await self.jutsu_scraper.parse_episode_page(first_episode_url, page, anime_slug)
            if not base_episode_data:
                await browser.close()
                return {"status": "error", "reason": "failed to parse first episode"}
            
            anime_title_rus = base_episode_data['anime_title_rus']

            # 3. Получаем метаданные с Jikan
            metadata = await self.metadata_scraper.get_anime_details(anime_title_rus)

            # 4. Сохраняем все в базу
            with db_manager.session_scope() as session:
                # Создаем или находим тип контента
                content_type_obj, _ = db_manager.get_or_create(session, ContentType, name=(metadata.get('type') if metadata else 'Unknown'))
                
                # Создаем запись аниме
                anime_obj = Anime(
                    slug=anime_slug,
                    title_rus=anime_title_rus,
                ...
                session.flush() # Получаем ID для anime_obj
                anime_id = anime_obj.id

                # Итерируемся по всем сезонам и эпизодам
                for season_num, episode_links in seasons_with_links.items():
                    ...
                    for link in episode_links:
                        # Если это первый эпизод, мы его уже спарсили
                        if link == first_episode_url:
                            episode_data = base_episode_data
                        else:
                            episode_data = await self.jutsu_scraper.parse_episode_page(link, page, anime_slug)
    ...
    async def add_bulk_anime(self, limit: int):
        """Добавляет указанное количество новых аниме."""
        print(f"[START] Запуск массового добавления. Лимит: {limit}")
        all_slugs_on_site = await self.jutsu_scraper.get_all_anime_slugs()
        ...
        for slug in slugs_to_add:
            result = await self.add_specific_anime(slug)
    ...
    async def run_continuous_scraping(self):
        """Бесконечный цикл для постоянного поиска и добавления нового контента."""
        print("[START] Запуск непрерывного скрапинга...")
        while True:
            print("\n" + "="*50)
            print(f"[{asyncio.get_event_loop().time()}] Новая итерация непрерывного скрапинга.")
            
            # Ищем и добавляем все новые аниме
            await self.add_bulk_anime(limit=10000) # Очень большой лимит, чтобы добавить все
            
            # TODO: В будущем можно добавить логику проверки новых эпизодов для существующих аниме
            
            sleep_duration = 3600 # 1 час
            print(f"[*] Все новые аниме обработаны. Следующая проверка через {sleep_duration / 60} минут.")
            print("="*50 + "\n")
