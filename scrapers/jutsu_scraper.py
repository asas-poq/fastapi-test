...
class JutsuScraper:
    """
    Скрапер для jut.su.
    - Находит все аниме на сайте.
    - Для каждого аниме находит все сезоны и эпизоды.
    - Парсит страницу эпизода для получения детальной информации.
    """
    def __init__(self):
        self.base_url = config.JUTSU_BASE_URL
        self.output_dir = config.POSTERS_OUTPUT_DIR

    async def get_all_anime_slugs(self):
        ...
        async with aiohttp.ClientSession() as session:
            while True:
                ...
                try:
                    async with session.get(catalog_url, timeout=15) as response:
                        if response.status != 200:
                            print(f"[*] Достигнут конец каталога на странице {page_num}.")
                            break
                        
                        html_content = await response.text()
                        soup = BeautifulSoup(html_content, 'html.parser')
                        
                        links = soup.select('a.animes_small_box')
                        if not links:
                        ...
                        for link in links:
                            href = link.get('href')
                            if href and not href.startswith(('/user/', '/news/')):
                                slug = href.strip('/').split('/')[-1]
                                all_slugs.add(slug)
                        
                        print(f"  [+] Найдено {len(links)} аниме. Всего уникальных: {len(all_slugs)}")
                        page_num += 1
    ...
    async def get_all_episode_links_for_anime(self, anime_slug):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(anime_page_url, timeout=15) as response:
                    ...
                    if season_tabs:
                        ...
                        # Если вкладок сезонов нет, считаем все одним сезоном
    ...
    async def parse_episode_page(self, episode_url, page, anime_slug):
        ...
        # Динамическое извлечение переменных из window
        try:
        ...
        data['next_episode_url'] = urljoin(self.base_url, next_link) if next_link else None
